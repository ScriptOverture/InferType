...
export function getAllParametersType(
  parasms: ParameterDeclaration[],
 ...
 * @returns
 */
export function getPropertyAssignmentType(
  scope: Scope,
  iType: Expression,
): Variable | undefined {
  if (!iType) return
...
function inferElementAccessExpression(
  ...
  const expressionVariable = getPropertyAssignmentType(
    scope,
...
function inferConditionalExpressionType(
  ...
  const whenTrueVariable = getPropertyAssignmentType(
    scope,
    unwrapParentheses(whenTrueNode),
  )!
  const whenFalseVariable = getPropertyAssignmentType(
    scope,
...
export function uniqueBaseType(types: BasicType[]): BasicType[] {
  ...
  types.forEach((item, index) => {
    const state = types
      .slice(index + 1)
      .some((el) => item.constructor === el.constructor)
    if (!state) {
      result.push(item)
    }
  })
